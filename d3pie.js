...
var labels = {
	...
	computeLinePosition: function(pie, i) {
		...
		var widthOffset = pie.outerLabelGroupData[i].w; // TODO check
		...
		} else if( pie.options.labels.lines.style === "pointer" ){
			var corX, corY, corX2;
			
			corY = y3 + 10;
			originCoords = math.rotate(pie.pieCenter.x, pie.pieCenter.y - pie.outerRadius + 20, pie.pieCenter.x, pie.pieCenter.y, angle);
	...
	addLabelLines: function(pie) {
		var lineGroups = pie.svg.append("g", "." + pie.cssPrefix + "pieChart") // meaning, BEFORE .pieChart
		...
		lineGroup.append("defs").append("marker")
			.attr("id", "circ")
			.attr("markerWidth", 4)
			.attr("markerHeight", 4)
			.attr("refX", 2)
...
var segments = {
	...
	addSegmentEventHandlers: function(pie) {
		...
		var _maxSegIdx = 0;
		var _value = 0;
		for(var _i = 0 ; _i < pie.options.data.content.length ; _i++ ){
			if( pie.options.data.content[_i].value > _value ){
				_value = pie.options.data.content[_i].value;
				_maxSegIdx = _i;
			}
		}
		
		setTimeout(function(){
			var segment = d3.select(arc[0][_maxSegIdx]).select("path");
			segments.openSegment(pie, segment.node() );
		},1000);
